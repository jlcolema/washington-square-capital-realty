...
class Zo_Recent_Posts_Widget_With_Thumbnails extends WP_Widget
{
    ...
    function __construct()
    {
        ...
        parent::__construct(
            ...
            esc_html__('@JWS Post Thumbnail', 'consbie'), // Name
            array('description' => esc_html__('List of your site&#8217;s most recent posts, with clickable title and thumbnails.', 'consbie'),) // Args
        );
                        ...
                        <div class="tb-recent-detail">
                            <div class="post-tumb">
                                <?php if (!$hide_thumbnail) { ?>
                                    <?php echo jws_get_post_thumbnail('175x190'); ?>
                                <?php } ?>
                            </div>
                            <div class="post-content">
                                ...
                                <?php if (!$hide_author) { ?>
                                    <div class="tb-post-author">
                                        <p>
                                            <?php
                                            echo esc_html__('By ', 'consbie') . '<span>' . get_the_author() . '</span>'
                                            ?>
                                        </p>
                                        <span class="comment">
                                ...
                                <?php if (!$hide_button) { ?>
                                    ...
                                    <a href="<?php the_permalink(); ?>"
                                       class="btn btn-default"><?php esc_html_e('Read More', 'consbie'); ?></a>
    ...
    function form($instance)
    {
        ...
        ?>
        <p><input class="checkbox" type="checkbox" <?php checked($hide_thumbnail); ?>
                  id="<?php echo esc_attr($this->get_field_id('hide_thumbnail')); ?>"
                  name="<?php echo esc_attr($this->get_field_name('hide_thumbnail')); ?>"/>
            <label for="<?php echo esc_attr($this->get_field_id('hide_thumbnail')); ?>"><?php esc_html_e('Do not show hide_thumbnail?', 'consbie'); ?>
                <em><?php esc_html_e('Make sure you set a default thumbnail for posts without a tille.', 'consbie'); ?></em></label>
        </p>

        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'consbie'); ?></label>
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>"
                   name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text"
                   value="<?php echo esc_attr($title); ?>"/></p>

        <p>
            <label for="<?php echo esc_attr($this->get_field_id('number_posts')); ?>"><?php esc_html_e('Number of posts to show:', 'consbie'); ?></label>
            <input id="<?php echo esc_attr($this->get_field_id('number_posts')); ?>"
                   name="<?php echo esc_attr($this->get_field_name('number_posts')); ?>" type="text"
                   value="<?php echo esc_attr($number_posts); ?>" size="3"/></p>

        <p><input class="checkbox" type="checkbox" <?php checked($show_date); ?>
                  id="<?php echo esc_attr($this->get_field_id('show_date')); ?>"
                  name="<?php echo esc_attr($this->get_field_name('show_date')); ?>"/>
            <label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>"><?php esc_html_e('Display post date?', 'consbie'); ?></label>
        </p>

        <p><input class="checkbox" type="checkbox" <?php checked($hide_title); ?>
                  id="<?php echo esc_attr($this->get_field_id('hide_title')); ?>"
                  name="<?php echo esc_attr($this->get_field_name('hide_title')); ?>"/>
            <label for="<?php echo esc_attr($this->get_field_id('hide_title')); ?>"><?php esc_html_e('Do not show title?', 'consbie'); ?>
                <em><?php esc_html_e('Make sure you set a default title for posts without a tille.', 'consbie'); ?></em></label>
        </p>

        <p><input class="checkbox" type="checkbox" <?php checked($hide_author); ?>
                  id="<?php echo esc_attr($this->get_field_id('hide_author')); ?>"
                  name="<?php echo esc_attr($this->get_field_name('hide_author')); ?>"/>
            <label for="<?php echo esc_attr($this->get_field_id('hide_author')); ?>"><?php esc_html_e('Do not show author?', 'consbie'); ?>
                <em><?php esc_html_e('Make sure you set a default author for posts without a tille.', 'consbie'); ?></em></label>
        </p>

        <p><input class="checkbox" type="checkbox" <?php checked($hide_content); ?>
                  id="<?php echo esc_attr($this->get_field_id('hide_content')); ?>"
                  name="<?php echo esc_attr($this->get_field_name('hide_content')); ?>"/>
            <label for="<?php echo esc_attr($this->get_field_id('hide_content')); ?>"><?php esc_html_e('Do not show content?', 'consbie'); ?>
                <em><?php esc_html_e('Make sure you set a default content for posts without a content.', 'consbie'); ?></em></label>
        </p>

        <p><input class="checkbox" type="checkbox" <?php checked($hide_button); ?>
                  id="<?php echo esc_attr($this->get_field_id('hide_button')); ?>"
                  name="<?php echo esc_attr($this->get_field_name('hide_button')); ?>"/>
            <label for="<?php echo esc_attr($this->get_field_id('hide_button')); ?>"><?php esc_html_e('Do not show button read more?', 'consbie'); ?>
                <em><?php esc_html_e('Make sure you set a default button for posts without a button.', 'consbie'); ?></em></label>
        </p>
 ...
 * @since 1.0
 */
function jws_register_recent_posts_widget_with_thumbnails()
{
...
add_action('widgets_init', 'jws_register_recent_posts_widget_with_thumbnails');