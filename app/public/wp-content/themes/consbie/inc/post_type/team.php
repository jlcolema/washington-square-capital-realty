...
if ( ! defined( 'ABSPATH' ) ) {
    ...
    function jws_register_teams() {
		$labels = array(
			'name'                => _x( 'teams', 'Post Type General Name', 'consbie' ),
			'singular_name'       => _x( 'teams', 'Post Type Singular Name', 'consbie' ),
			'menu_name'           => esc_html__( 'Teams', 'consbie' ),
		...
		$args = array(
			'label'               => esc_html__( 'teams', 'consbie' ),
		    'labels'              => $labels,
        ...
        if(function_exists('custom_reg_post_type')){
        	custom_reg_post_type( 'teams', $args );
        }

		/**
		 * Create a taxonomy category for teams
		 *
		...
		$labels = array(
			'name'					=> _x( 'teams Categories', 'Taxonomy plural name', 'consbie' ),
			'singular_name'			=> _x( 'teams Category', 'Taxonomy singular name', 'consbie' ),
			'search_items'			=> esc_html__( 'Search Categories', 'consbie' ),
			'popular_items'			=> esc_html__( 'Popular teams Categories', 'consbie' ),
			'all_items'				=> esc_html__( 'All teams Categories', 'consbie' ),
		...
		$args = array(
            ...
            'query_var'         => true,
            'rewrite'           => array( 'slug' => 'teams_cat' ),
		);
        

        if(function_exists('custom_reg_taxonomy')){
            custom_reg_taxonomy( 'teams_cat', array( 'teams' ), $args  );
        }
        ...
        $args = array(
            ...
            'query_var' => true,
            'rewrite' => array( 'slug' => 'teams_tag' ),
        );
        
        if(function_exists('custom_reg_taxonomy')){
            custom_reg_taxonomy( 'teams_tag', array( 'teams' ), $args  );
        }

	};
add_action( 'init', 'jws_register_teams', 1 );